...
var locationClearCache = function(url) {
	location = /\?/.test(url)? url+"&v000000011" : url+"?v000000011";
}
if(debug) {
...
function ormatDate(dateNum,bool,sign){
	...
	if (bool) {
		...
		return date.getFullYear()+sign+fixZero(date.getMonth()+1,2)+sign+fixZero(date.getDate(),2)+"   "+fixZero(date.getHours(),2)+":"+fixZero(date.getMinutes(),2)+":"+fixZero(date.getSeconds(),2);
...
Scr.prototype.init = function(){
	this.box.on('scroll', this.onScr);
}

Scr.prototype.destory = function(){
	this.box.off('scroll', this.onScr);
}
...
function wx_init(opt,errorCallback){
	//console.log(1)
	var targetUrl = location.toString().split('#')[0];
