...
$(function() {
	...
	var iscouponCode = true,number,cash;	// 防止重复兑换优惠券
	// $.showPreloader();
	// init
	common.ajaxApi(url+ "api/My/coupon",
		function(res){
			number = res.data.coupon.length;
			var cash_len = res.data.cash.length;
			if(!number) {
				$("#boxNum").html("");
			...
			if(!cash_len) {
				$("#cashboxNum").html("");
				$("#cashorpBox").append('<div class="air">我已经准备好姿势，<span class="col-blue ind">等你来领</span></div>')
				$("#cashhistory").hide();
			}else {
				$("#cashboxNum").html('有<b>'+cash_len+'</b>张现金券可用');
			}
			$("#orpBox ul").remove();
			$("#cashorpBox ul").remove();
			// 输出
			outCou(res.data.coupon,$("#orpBox"));
			outCou(res.data.cash,$("#cashorpBox"));
		},function(res){
			$.alert(res.msg)
	...
	function outCou(couArr,obj) {
		...
		for(var i = 0; i < number; i++ ) {
			...
			ul.append('<li class="proper com-select-code"></li>');
			ul.appendTo(obj);
		}
	}
	// 过期输出
	...
	//  滚动加载现金券函数   boolSex防止重复请求  firstbool是否第一次加载  couponPage优惠券页数  插入优惠的对象
	...
	// 去优惠券说明
	$(document).on("tap","#explain,#cashexplain",function(){
	...
	// 点击兑换背后的蒙版
	$(document).on("touchstart",".ordeform-time,.icon-cancel",function(e){
		e.stopPropagation();
		e.preventDefault();
		$("#cancel").trigger("tap")
	})
	...
	$(document).on("tap",".com-select-code",function(){
